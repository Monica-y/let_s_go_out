...
public class AdminDaoImpl implements AdminDao{
    ...
    public boolean addUser(User u) throws Exception {
        String sql1 = "select max(WebAdmin.userID) from WebAdmin";
    ...
    public boolean deleteUser(int uid) throws Exception {
        String sql = "delete from kongming234.dbo.WebAdmin where userID = ? ";
        this.preparedStatement = this.conn.prepareStatement(sql);
        this.preparedStatement.setInt(0,uid);
        int result = this.preparedStatement.executeUpdate();
        this.preparedStatement.close();
        if(result == 0) {
            return false;
    ...
    public User getUserByName(String uname) throws Exception {
        User u = new User();
        String sql = "select * from kongming234.dbo.WebAdmin where name = ?";
        this.preparedStatement = this.conn.prepareStatement(sql);
